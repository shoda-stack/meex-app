    ...
    <div className="min-h-screen bg-[#f3b32a] text-black font-bold p-6 flex flex-col items-center text-center font-sans">
      ...
      {view === 'register' && (
        <div className="w-full max-w-sm bg-black text-white p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
          ...
          <form onSubmit={handleRegister} className="space-y-6 text-left text-black">
            ...
            <button type="submit" disabled={loading} className="w-full bg-white text-black p-5 text-xl font-black mt-4 uppercase">{loading ? "送信中..." : "Ticket発行"}</button>
          </form>
      ...
      {view === 'admin' && (
        <div className="w-full max-w-sm bg-white p-8 border-[6px] border-black shadow-[14px_14px_0px_0px_rgba(0,0,0,1)]">
          <h2 className="text-2xl mb-6 italic border-b-2 border-black pb-2 uppercase tracking-widest text-black">Scanner</h2>
          <div id="reader" className="w-full mb-4 bg-black min-h-[200px] overflow-hidden rounded-lg border-4 border-black text-black"></div>
          {adminStatus === "待機中" ? (
            ...
            <div className="space-y-4">
              <div className="text-3xl border-b-4 border-black pb-2 text-black">{scanResult.name} 様</div>
            ...
            <div className="space-y-6">
              <div className="text-8xl text-green-600 italic">DONE</div>
            ...
            <div className="p-10 italic text-2xl animate-pulse text-black">{adminStatus}</div>
          )}
